...
The trickiest part of the whole process was actually merging the GitHub workflows, so it wasn't related to Bun at all. The way I wanted my repository set up is that any push to `main` would have to be submitted as a pull request and approved. But, the pull request should only be approved when the build was successful. But I only wanted the workflow to publish GitHub pages when the pull request was approved and merged. Since I wanted to run the build essentially on every push, including merged pull requests, I could have created two separate, but nearly identical actions, both which would install dependencies and build my projects. But you know me by now! I want to reuse my code, no matter how insignificant. So, I created a single workflow which handles the building and will only publish if merged to `main`. For this I had to add this line to my `deploy` job:
...
Believe me. I was so frustrated to find out that Bun doesn't natively support linting. There's currently an open issue in the [Bun issues](https://github.com/oven-sh/bun/issues) page requesting the feature, but who knows if it will ever be implemented. With Anthropic's vast pool of AI resources, you'd think this would be an easy task for them to handle.
...
I turned from being a skeptic to having fun in the Bun.
